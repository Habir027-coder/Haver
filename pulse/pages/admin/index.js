...
export default function AdminPage(){
  ...
  useEffect(()=>{ fetch('/pulse/api/auth/me').then(r=>r.json()).then(d=>{ if (d.user) setUser(d.user); }).catch(()=>{}); },[]);

  async function login(e){
    e.preventDefault();
    const res = await fetch('/pulse/api/auth/login', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ email, password }) });
  ...
  async function loadPunches(){
    setLoading(true);
    const res = await fetch('/pulse/api/punches');
